...
<Steps>
  <Step title="Read the options">
    `GET /launch-options` and look under `launchpads.pons`: who may launch, the creator tax range, and the payout assets holders can be paid in. Pairs are under `robinhood` in `GET /pairs`. A token not in the list can be checked with `GET /pons/assets/{address}`.
  </Step>
  <Step title="Prepare">
    `POST /pons/launches/prepare` with `name`, `symbol`, `creatorWallet` and optionally `pairToken` (an `address` from `robinhood` in `/pairs`; the zero address or omitted means ETH), `devBuy` (ETH, decimal string) and `creatorTaxBps`. This is the one launch that still signs a message first: launching here is invite-only, and the signature is how the invite is checked before anything is spent. The message carries a `launches:` line saying the name, ticker, pair, creator tax and first buy in words; read it, then sign `message` with `personal_sign`.
  </Step>
  <Step title="Submit">
  ...
  <Step title="Find the launch">
    Once the launch transaction has succeeded, `GET /pons/launches/{txHash}` returns the token address, and the `walletKey` again if you lost it.
  </Step>
  <Step title="Register the strategy">
    `POST /pons/register/prepare` with `creatorWallet` and `txHash`; sign. Then `POST /pons/register/submit` with `walletKey`, `name`, `symbol`, `imageUrl`, your strategy `config`, `ts` and `signature`. If `registered` is false with a reason, the launch is not indexed yet: wait a few seconds and retry. The `config` is the same shape as a Solana strategy launch: a paying coin names its asset in `payoutMint` and up to four more in `payoutMintsExtra`, each a `0x` address from the venue's `payoutAssets` or any token `GET /pons/assets/{address}` says is payable.
  </Step>
</Steps>
