...
<Steps>
  <Step title="Read the options">
    `GET /launch-options` gives the fee tiers, the three strategy presets as ready-made config, the airdrop-only config and the payout assets. `GET /pairs` lists the pairs (SOL is `So11111111111111111111111111111111111111112`); `GET /markets` the perp markets and their leverage caps.
  </Step>
  <Step title="Prepare">
...
- `rewardMode` decides what profits do: `burn` buys and burns the coin, `holders` airdrops the coin, `backed` airdrops `payoutMint`, a tokenised stock or major from `payoutAssets` or any Solana mint with a real market. `payoutMint2` adds a second asset, half each. Paying coins need `holderPayoutsEnabled` to be true.
